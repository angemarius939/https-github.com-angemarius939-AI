  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-slate-50/50">
      <div className="flex-1 flex flex-col md:flex-row h-full overflow-hidden">
        ...
        {/* SIDEBAR: Configuration & History (Left) */}
          ...
          <div className="p-8 space-y-10 overflow-y-auto h-full scrollbar-hide">
            <div className="flex justify-between items-center">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Academic Engine Active</span>
                </div>
              </div>
              {courseContent && (
                <button onClick={() => setIsMobileSidebarOpen(false)} className="md:hidden p-2 text-stone-400">
                  <X className="w-6 h-6" />
                </button>
              )}
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-900/40 px-1">Urwego (Level)</label>
                  ...
                  {(['beginner', 'intermediate', 'advanced'] as CourseLevel[]).map((l) => (
                    <button
                      ...
                      className={`py-2 text-[10px] font-black uppercase tracking-widest rounded-xl border-2 transition-all ${
                        level === l ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-white border-emerald-50 text-emerald-800 hover:border-emerald-200'
                      }`}
                    >
                      {l === 'beginner' ? 'Intangiriro' : l === 'intermediate' ? 'Hagati' : 'Icyisumbuye'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-900/40 px-1">Ingingo y'Isomo</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-emerald-50/30 border-2 border-emerald-50 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none font-bold text-emerald-950 placeholder:text-stone-300"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-900/40 px-1">Igihe</label>
                  <input type="text" value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="Ukwezi 1" className="w-full p-4 bg-emerald-50/30 border-2 border-emerald-50 rounded-2xl outline-none font-bold text-sm" />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-900/40 px-1">Ibisabwa</label>
                  <input type="text" value={prerequisites} onChange={(e) => setPrerequisites(e.target.value)} placeholder="Nta na kimwe" className="w-full p-4 bg-emerald-50/30 border-2 border-emerald-50 rounded-2xl outline-none font-bold text-sm" />
                </div>
              </div>
              ...
              <Button 
                ...
                isLoading={isLoading} 
                disabled={!topic.trim()} 
                className="w-full h-16 rounded-[24px] text-lg font-black uppercase tracking-[0.1em] shadow-xl shadow-emerald-600/20"
              >
                <GraduationCap className="w-6 h-6 mr-3" />
                Tegura Isomo
              </Button>
              <ProgressBar isLoading={isLoading} label="Irimo gutegura isomo ryawe..." duration={8000} />
            </div>

            {history.length > 0 && (
              <div className="pt-8 border-t border-emerald-50 space-y-4">
                <h3 className="text-[10px] font-black text-emerald-900/40 uppercase tracking-[0.2em] flex items-center gap-2">
                  <History className="w-4 h-4" /> Amateka
                </h3>
                <div className="space-y-3">
                  {history.map((item) => (
                    <button 
                      key={item.id} 
                      onClick={() => { setCourseContent(item.content); setSources(item.sources || []); setTopic(item.topic); }} 
                      className={`w-full text-left p-5 rounded-2xl border transition-all relative overflow-hidden group ${
                        courseContent === item.content ? 'bg-emerald-600 border-emerald-600 text-white shadow-xl' : 'bg-white border-stone-100 hover:border-emerald-200'
                      }`}
                    >
                      <div className="font-black text-sm uppercase tracking-tight truncate pr-8">{item.topic}</div>
                      <div className={`text-[10px] font-bold mt-1 uppercase tracking-widest ${courseContent === item.content ? 'text-emerald-100' : 'text-stone-400'}`}>
                        {item.level} &bull; {new Date(item.timestamp).toLocaleDateString()}
                      </div>
                      <ChevronRight className={`absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-transform ${courseContent === item.content ? 'text-white translate-x-1' : 'text-stone-300'}`} />
                    </button>
                  ))}
                </div>
              </div>
          ...
          {courseContent && (
            <div className="bg-white/80 backdrop-blur-xl border-b border-emerald-50 p-5 px-8 flex justify-between items-center z-30 sticky top-0 shadow-sm">
               <div className="flex items-center gap-4">
                 <button onClick={() => setIsMobileSidebarOpen(true)} className="md:hidden p-2.5 bg-emerald-50 rounded-xl text-emerald-600">
                   <Menu className="w-6 h-6" />
                 </button>
                 <div>
                    <h1 className="text-xl md:text-2xl font-black text-emerald-950 tracking-tighter uppercase truncate max-w-[200px] md:max-w-md">{topic}</h1>
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">{level} level masterclass</span>
                 </div>
               </div>
               <div className="flex gap-2">
                  <button onClick={handleCopy} className="p-3 bg-stone-50 text-stone-400 hover:text-emerald-600 hover:bg-emerald-100 rounded-xl transition-all shadow-sm">
                    {copied ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                  </button>
                  <button onClick={() => window.print()} className="p-3 bg-stone-50 text-stone-400 hover:text-emerald-600 hover:bg-emerald-100 rounded-xl transition-all shadow-sm">
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto scroll-smooth custom-scrollbar">
              {!courseContent ? (
                <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-12 animate-in fade-in duration-1000">
                  <div className="relative">
                    <div className="absolute -inset-10 bg-emerald-500/10 rounded-full blur-[80px] animate-pulse"></div>
                    <div className="relative w-40 h-40 bg-white rounded-[48px] shadow-2xl flex items-center justify-center border border-emerald-50">
                  ...
                  <div className="space-y-6 max-w-xl">
                    ...
                    <p className="text-stone-500 font-medium text-lg md:text-xl leading-relaxed">
                      Tegura isomo rirambuye ku ntego wihaye. Hitamo imwe mu ngero hano hasi cyangwa wandike ingingo yawe ibumoso.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-4xl px-4">
                    {exampleTopics.map((ex, i) => (
                      <button 
                        ...
                        className="p-8 bg-white rounded-[40px] border border-stone-100 hover:border-emerald-300 hover:shadow-2xl hover:-translate-y-2 transition-all text-left group"
                      >
                        <div className="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                          <ex.icon className="w-7 h-7" />
                        </div>
                        <span className="font-black text-emerald-950 uppercase tracking-tight block text-lg mb-2">{ex.label}</span>
                        <div className="flex items-center text-[10px] font-black text-emerald-500 uppercase tracking-widest gap-2">
                          Koresha iyi ngero <ArrowRight className="w-4 h-4" />
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div className="max-w-4xl mx-auto p-6 md:p-12 pb-40 space-y-20">
                  ...
                  <header className="bg-emerald-950 rounded-[64px] p-10 md:p-20 text-white shadow-2xl relative overflow-hidden group">
                     <div className="absolute top-0 right-0 p-40 bg-white opacity-[0.03] rounded-full transform translate-x-20 -translate-y-20 blur-3xl group-hover:scale-110 transition-transform duration-1000"></div>
                     <div className="relative z-10 space-y-10">
                        <div className="flex flex-col md:flex-row md:items-center gap-6 justify-between">
                          <div className="inline-flex items-center gap-3 px-6 py-2 bg-white/10 rounded-full text-[11px] font-black uppercase tracking-[0.25em] backdrop-blur-sm border border-white/10">
                            <Map className="w-4 h-4 text-emerald-400" /> Roadmap y'Isomo
                          </div>
                          <div className="flex gap-4 text-xs font-bold text-emerald-200 uppercase tracking-widest">
                            <div className="flex items-center gap-2"><Clock className="w-4 h-4" /> {duration || 'Ubushake'}</div>
                            <div className="flex items-center gap-2"><Target className="w-4 h-4" /> {level}</div>
                          </div>
                        </div>
                  ...
                  <section className="space-y-12">
                     <div className="flex items-center gap-4 mb-8">
                        ...
                        <span className="text-[11px] font-black uppercase tracking-[0.4em] text-emerald-950">Intambwe z'isomo</span>
                        <div className="h-0.5 flex-1 bg-emerald-100"></div>
                     </div>
                     ...
                     <div className="space-y-16">
                        {parsedSections.map((section, idx) => {
                          ...
                          return (
                            <div key={section.id} id={section.id} className="relative group animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: `${idx * 0.1}s` }}>
                              {/* Connector Line */}
                              {idx < parsedSections.length - 1 && (
                                <div className="absolute left-8 top-16 bottom-0 w-1 bg-gradient-to-b from-emerald-100 to-transparent -mb-16"></div>
                              )}
                              
                              <div className="flex gap-8 md:gap-12">
                                 <div className={`w-16 h-16 shrink-0 rounded-[24px] ${styles.bg} border-2 ${styles.border} flex items-center justify-center text-xl font-black text-emerald-900 shadow-sm relative z-10 group-hover:scale-110 transition-transform`}>
                                   {idx + 1}
                                 </div>
                                 
                                 <div className={`flex-1 bg-white rounded-[40px] shadow-sm border border-stone-100 overflow-hidden transition-all duration-500 hover:shadow-2xl hover:border-emerald-100 ${activeSection === section.id ? 'ring-4 ring-emerald-500/5' : ''}`}>
                  ...
                  <div className="space-y-2">
                    {parsedSections.map((s, idx) => (
                      <button 
                        ...
                        className={`w-full text-left px-5 py-4 text-[10px] font-black rounded-2xl transition-all flex items-center gap-3 group ${activeSection === s.id ? 'bg-emerald-600 text-white shadow-xl translate-x-1' : 'text-stone-400 hover:bg-emerald-50 hover:text-emerald-700'}`}
                      >
                        ...
                        <span className="truncate uppercase tracking-tight leading-tight">{s.title}</span>
                      </button>
                    ))}
                  </div>
